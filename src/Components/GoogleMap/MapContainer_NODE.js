...
import { mapAction, initMaterial, getCardinal } from '../../Store/Actions/miscActions'
import QuickMenu from '../Misc/QuickMenu'
...
export class MapContainer extends Component{
    ...
    changeMarker(busId, bus){
        ...
        if(found === true){
            ...
            if(gpsState && gpsState === true){
                ...
                myBus.busName   = bus.busName;
                myMarker.setPosition(busCoords);
                myMarker.setTitle(bus.busName);
                //if(busDetails !== ''){this.setInfoDetails(myinfoWindow, bus, myMarker);}
                // console.log('chandhdfhsghsddfgdgd', myinfoWindow);
    ...
    initMarkers(){
        ...
        var busReflist = FireDB.ref('/MyRide/BusGEO/');
    ...
    setInfoDetails = (infoWindow, bus, busMarker) => {
        const { google, getCardinal }    = this.props;
        ...
        if(bus){
            let AddressName     = '',
                ...
                directionCoord  = (bus.heading) ? getCardinal(bus.heading) : null,
                position        = {
            ...
            function GeoDetails(results, status){
                ...
                innerHTML += '<h5 class="info-title"><i class="material-icons">directions_bus</i> '+ bus.busName +'</h5>';
                ...
                innerHTML += (directionCoord !== null) ? '<span><i class="material-icons tiny">directions</i> '+ directionCoord +'</span>' : '';
    ...
    addEffect = (e) =>{
        let element = document.querySelector("#extraMenu");
...
const mapDispatchToProps = (dispatch) => {
    return{
        mapAction: (action) => dispatch(mapAction(action)),
        initMaterial: (direction) => dispatch(initMaterial(direction)),
        getCardinal: (angle) => dispatch(getCardinal(angle))
    }
}
